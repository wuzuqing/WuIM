...
public class AlbumActivity extends BaseActivity implements
    ...
    private void initializeArgument() {
        ...
        isCrop = argument.getBoolean(Album.KEY_INPUT_FILE_CROP);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            ...
            case 3:
                if (resultCode == RESULT_OK && !mCheckedList.isEmpty() && outUri != null) {
                    mCheckedList.get(0).setPath(GetPathFromUri4kitkat.getPath(this, outUri));
                    if (sResult != null) sResult.onAction(mCheckedList);
                    finish();
                } else {
                    callbackCancel();
                }
...
//                mView.notifyItem(position);
                ...
                setCheckedCount();
                if (isCrop) {
                    outUri = AlbumUtils.startPhotoZoom(AlbumActivity.this, 3, albumFile.getPath());
                } else {
                    callbackResult();
                }
                break;
