...
public class Luban {
    ...
    public static Luban get(Context context) {
        if (INSTANCE == null)
            INSTANCE = new Luban(new File(FileUtils.getImageFilePath()));
        return INSTANCE;
    }

    public Luban cleanCache() {
        FileUtils.delete(mCacheDir, false);
        return INSTANCE;
    }
     ...
     * 压缩单张图片 RxJava 方式
     */
    private void compressWithRx(File file, Consumer<File> action) {
        Luban.get(BaseApplication.getAppContext())
                .load(file)
                .putGear(Luban.THIRD_GEAR)
                .asObservable()
                ...
                .onErrorResumeNext((Function<Throwable, Observable<? extends File>>) throwable -> {
                    return Observable.empty();
                })
                .subscribe(action);
    }
